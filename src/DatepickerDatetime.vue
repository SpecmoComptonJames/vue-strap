    ...
    export default {
        ...
        watch: {
            ...
            value(val, old) {
                ...
                var _self = this;
                if (val) {
                    var x = _self.extractDateParts(val);
                    this.currDate = x.date;
                }
            }
        },
        computed: {
        ...
        methods: {
            extractDateParts: function (valueIn) {
                var format = JSON.parse(JSON.stringify(this.format));
                var pos;
                var valuePieces;
                var month;
                var year;
                ...
                if (valueIn instanceof Date) {
                    value = valueIn.toLocaleString();
                    format = 'dd/MM/yyyy';
                    if (value.indexOf(',') > -1) {
                        //remove comma (time)
                        var removeme = value.split(',');
                        value = removeme[0];
                    }
                } else {
                    value = valueIn;
                ...
                if (valid) {
                    ...
                    //assemble string date
                    var reassemble = function(idx, pos, dateString) {
                        switch(pos[idx]) {
                            case "MM":
                            case "mm":
                                var mon = '00' + month;
                                dateString = dateString + mon.substring(mon.length-2);
                                break;
                            case "dd":
                                var day = '00' + month;
                                dateString = dateString +  day.substring(day.length-2);
                                break;
                            case "yyyy":
                            case "YYYY":
                    ...
                    dateString = reassemble(0,pos,dateString);
                    dateString = reassemble(1,pos,dateString);
                    dateString = reassemble(2,pos,dateString);
                }
                return {month: month, year: year, day: day, valid: valid, date: date, dateString: dateString};
            },
            validate() {
                var valid = true;
                if (!this.canValidate) {
            ...
            daySelect(day) {
                if (day.sclass === 'datepicker-item-disable') {
                    ...
                    //this.val = this.stringify(this.currDate);
                    //var x = this.extractDateParts(this.currDate);
                    //this.val = x.dateString;
                    this.value = day.date;
                    this.displayDayView = false;
                }
            },
